import express from "express";
import {
  getVitals,
  addVital,
  getVaccines,
  addVaccine,
  generateCertificate,
  getMedications,
  addMedication,
  deleteMedication,
  getMedicalRecords,
  addMedicalRecord,
  getAllergies,
  addAllergy,
  deleteVaccine,
  updateVaccine,
  deleteVital,
  updateVital,
  updateMedication,
  deleteAllergy,
  updateAllergy,
} from "../controllers/healthController.ts";
import { protect } from "../middleware/authMiddleware.ts";

const router = express.Router();

router.route("/vitals/:petId").get(protect, getVitals).post(protect, addVital);
router.route("/vitals/:petId/:vitalId").delete(protect, deleteVital).put(protect, updateVital);

router.route("/vaccines/:petId").get(protect, getVaccines).post(protect, addVaccine);
router.route("/vaccines/:petId/:vaccineId").delete(protect, deleteVaccine).put(protect, updateVaccine);
router.post("/vaccines/:petId/:vaccineId/certificate", protect, generateCertificate);

router.route("/meds/:petId").get(protect, getMedications).post(protect, addMedication);
router.route("/meds/:petId/:medId").delete(protect, deleteMedication).put(protect, updateMedication);

router.route("/records/:petId").get(protect, getMedicalRecords).post(protect, addMedicalRecord);

router.route("/allergies/:petId").get(protect, getAllergies).post(protect, addAllergy);
router.route("/allergies/:petId/:allergyId").delete(protect, deleteAllergy).put(protect, updateAllergy);

export default router;
