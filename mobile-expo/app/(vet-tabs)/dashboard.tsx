...
export default function VetDashboard() {
  ...
  const fetchData = useCallback(async () => {
    try {
      const { stats: statsData, appointments: apptsData } =
        await vetHomeApi.getHomeData();
      ...
      const mappedStats = [
        ...
        },
        {
          ...
          value: String(statsData.totalPatients || 0),
          icon: PawPrint,
          color: "#10b981",
        },
        {
          label: "Avg Rating",
          value: String(statsData.avgRating || 4.8),
          icon: Star,
          color: "#f59e0b",
        },
        {
      ...
      const todayAppts = (apptsData || []).filter(
        (a: any) => a.date === todayStr && a.status === "confirmed",
      );
      ...
      >
        ...
        >
          ...
          <Text style={{ fontSize: 13, color: colors.textMuted, marginTop: 2 }}>
            {user?.clinic_name || "PetCare Clinic"} ·{" "}
            {user?.specialty || "Veterinarian"}
          </Text>
          <Image
            source={require("@/assets/peaking_cat.png")}
            style={{
              position: "absolute",
              right: 30,
              top: 33,
        ...
        <View style={{ paddingHorizontal: 20 }}>
          ...
          >
            ...
            <Pressable onPress={() => router.push("/(vet-tabs)/appointments")}>
              <Text
                style={{ fontSize: 14, color: colors.brand, fontWeight: "500" }}
              >
                View all
              </Text>
            </Pressable>
          ...
          {appointments.length === 0 ? (
            ...
            appointments.map((appt) => (
              ...
              >
                {appt.pet?.avatar_url ? (
                  <Image
                    source={{ uri: appt.pet.avatar_url }}
