...
import { View, ScrollView, Image, Pressable, Switch, Alert } from "react-native";
...
export default function ProfileScreen() {
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      userDiscoverApi
        .getSavedVetsCount()
        .then(setSavedVetsCount)
        .catch(() => {});
      userCommunityApi
        .getMyPosts()
        .then((p) => setMyPostsCount(p.length))
        .catch(() => {});
    }, [refreshUser]),
  );
        ...
        <View style={{ paddingHorizontal: 20, paddingBottom: 24 }}>
            ...
            >
              ...
              <View style={{ flexDirection: "row", alignItems: "center" }}>
                {[
                  ...
                  {
                    n: user?.isVerified ? "Verified" : "Active",
                    l: "Status",
                    icon: Heart,
                  },
                ].map((s, idx, arr) => (
          ...
          >
                ...
                <Pressable
                  ...
                  onPress={async () => {
                    if (user?.instagramUserId) {
                      Alert.alert(
                        "Instagram",
                        "Your account is synced. Do you want to disconnect?",
                        [
                          { text: "Cancel", style: "cancel" },
                          {
                            ...
                            onPress: async () => {
                              try {
                                ...
                                await userCommunityApi.disconnectInstagram();
                                await refreshUser();
                              } catch {
                  ...
                  style={{
                    ...
                    paddingVertical: 6,
                    borderRadius: 8,
                    backgroundColor: user?.instagramUserId
                      ? colors.bgSubtle
                      : colors.brand,
                    opacity: instagramLoading ? 0.6 : 1,
                  }}
                >
                  <Text
                    style={{
                      fontSize: 12,
                      fontWeight: "600",
                      color: user?.instagramUserId
                        ? colors.textSecondary
                        : "#fff",
                    }}
                  >
                    {instagramLoading ? "..." : user?.instagramUserId ? "Synced" : "Connect"}
                  </Text>
                </Pressable>
              </View>
            </View>
            ...
            >
              ...
              <Text
                style={{ fontSize: 14, fontWeight: "500", color: "#e11d48" }}
              >
                Sign Out
              </Text>
            </Pressable>
