...
const Sidebar = () => {
  ...
  const circleItems = [
    { label: "Circles", icon: CircleDot, href: "/circles" },
    { label: "Q&A", icon: HelpCircle, href: "/questions" },
  ];
  const isCirclesActive = pathname === "/circles" || pathname === "/questions";
    ...
    <aside className="w-64 bg-primary-900 text-white flex flex-col p-6 flex-none h-screen sticky top-0">
      ...
      <nav className="flex-1 overflow-y-auto pr-2 custom-scrollbar">
        <div className="flex min-h-full flex-col gap-1">
          ...
          <div className={`rounded-xl transition-all duration-200 ${isCirclesActive ? "bg-white/10" : "hover:bg-white/5"}`}>
            <Link
              href="/circles"
              className={`flex items-center gap-3 w-full p-3 rounded-xl transition-colors group ${
                isCirclesActive ? "text-white" : "text-white/60 hover:text-white"
              }`}
            >
              <CircleDot size={20} className={isCirclesActive ? "text-white" : "text-white/40 group-hover:text-white/70"} />
